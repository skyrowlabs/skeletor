...
module.exports = async function decide({ github, context, core }) {
  ...
  // `--base-branch {{BASE_BRANCH}} --release-branch {{RELEASE_BRANCH}}`, and
  // the second half of that condition is why this test compares two rendered
  // values rather than one.
  //
  // It used to test `base === '{{RELEASE_BRANCH}}'` alone. The sentence above
  ...
  // `--versioning {{VERSIONING}}`.** An earlier version of this comment argued
  ...
  // layer down: the comment knew about `{{BASE_BRANCH}}` and the condition did
  // not.
  //
  ...
  if (base === '{{RELEASE_BRANCH}}' && '{{RELEASE_BRANCH}}' !== '{{BASE_BRANCH}}') {
