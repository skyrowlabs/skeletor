...
module.exports = async function decide({ github, context, core }) {
  ...
  //
  // What that cost, in the order it was found. `base === '{{RELEASE_BRANCH}}'`
  ...
  // `'{{RELEASE_BRANCH}}' !== '{{BASE_BRANCH}}'` made the classifier reachable
  ...
  // `--versioning {{VERSIONING}}`.** Under `tag` the push releases nothing and
