...
def test_push_gates_the_branch_work_lands_on():
    """`push` must cover `{{BASE_BRANCH}}`, the branch work lands on.

    In a two-branch tree that is the point of contrast — `{{BASE_BRANCH}}` and
    not only `{{RELEASE_BRANCH}}`. **This tree was scaffolded with both set, and
