...
* ``{{SHELL_PACKAGE}}/bug.py`` and ``{{SHELL_PACKAGE}}/task.py``, which file into one.
...
``{{CLI}} bug`` with ``--label agent-bug`` as a default argument and nothing that
...
class Lane(NamedTuple):
    ...
    #: The subcommand that files into it — ``{{CLI}} bug``, ``{{CLI}} task``.
    key: str
