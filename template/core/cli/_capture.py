...
Underscore-prefixed because ``{{SHELL_PACKAGE}}/__init__.py`` discovers commands by
module name, and this module exports a command *factory* rather than a command.
A public name here would put a ``{{CLI}} capture`` in ``--help`` that nobody can
...
shipped one on ``{{CLI}} bug``, defaulting to the lane label and accepting
...
def capture_command(lane: Lane, help_text: str, epilog: str = "") -> click.Command:
    """Build ``{{CLI}} <lane.key>`` from the registry.
