...
binding and nothing else. See ``{{SHELL_PACKAGE}}/_capture.py`` for the mechanism, and
``{{CLI}} task`` for the intake that takes work which is not a defect.
"""
...
bug = capture_command(
    ...
    epilog=(
        "\b\n"
        "Example:\n"
        '  {{CLI}} bug "check docs is green over an empty scan" \\\n'
        ...
        'cd node_modules/x && ./{{CLI}} check docs" \\\n'
        '      --scope "the scan filter only; the fix flag is out" \\\n'
        '      --acceptance "the run reports the real document count; {{CLI}} test unit green"'
    ),
)
