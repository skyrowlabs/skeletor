"""``{{CLI}} task`` — capture small, self-contained work that is not a defect.

The third intake, and the one the other two leave a gap between. ``{{CLI}} bug``
...
``{{CLI}} bug`` exists to prevent, one size down.
...
task = capture_command(
    ...
    unable to answer `{{CLI}} bug --reproduce` honestly.
    """,
    epilog=(
        "\b\n"
        "Example:\n"
        '  {{CLI}} task "delete the ROLLOUT_PHASE_LEGACY flag" \\\n'
        ...
        '      --acceptance "no reference outside CHANGELOG; {{CLI}} test unit green"'
    ),
)
