...
**Setting up a project?** You probably want [`../AGENTS.md`](../AGENTS.md) — the
one-page instruction sheet — or the `/new-project` skill. This folder is the
longer-form material behind them.
...
and turning on a scheduled job — are worth doing but are not part of scaffolding.

Read the rationale when a rule looks like overhead: it names the incident behind
each one, which is the only fair basis for deciding whether it earns its place.
